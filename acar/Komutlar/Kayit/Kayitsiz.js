...
module.exports = {
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if(!uye) return message.channel.send(`Hata: Lütfen bir üye etiketleyin veya Id giriniz!  __Örn:__  \`${client.sistem.a_Prefix}kayıtsız @acar/ID\``).then(sil => sil.delete({timeout: 5000}));
    ...
    message.channel.send(`${uye} \`(${uye.id})\` üyesi, ${message.author} \`(${message.author.id})\` tarafından __kayıtsıza__ atıldı!`).catch().then(x => x.delete({timeout: 10000}));

...
module.exports = {
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if(!uye) return message.channel.send(`Hata: Lütfen bir üye etiketleyin veya Id giriniz!  __Örn:__  \`${client.sistem.a_Prefix}kayıt @acar/ID isim yaş\``).then(sil => sil.delete({timeout: 5000}));
    ...
    if(!isim || !yaş) return message.channel.send(`Hata: Lütfen tüm argümanları doldurunuz!  __Örn:__  \`${client.sistem.a_Prefix}kayıt @acar/ID isim yaş\``).then(sil => sil.delete({timeout: 5000}));
        ...
        let acarkayit = await message.channel.send(embed
       ...
       } if(uye.user.username.includes(acarveri.Tag)) uye.roles.add(acar.tagRolu); 
       if(uye.voice.channel) await uye.voice.setChannel(acarkanallar.kayitSonrasi);
       message.react("✅"); 
       return;
    }
};
...
module.exports = {
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if(!uye) return message.channel.send(`Hata: Lütfen bir üye etiketleyin veya Id giriniz!  __Örn:__  \`${client.sistem.a_Prefix}isim @transsly/ID isim yaş\``).then(sil => sil.delete({timeout: 5000}));
    ...
    if(!isim || !yaş) return message.channel.send(`Hata: Lütfen tüm argümanları doldurunuz!  __Örn:__  \`${client.sistem.a_Prefix}isim @acar/ID isim yaş\``).then(sil => sil.delete({timeout: 5000}));
